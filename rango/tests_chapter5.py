    ...
    def test_counts(self):
        ...
        for category in details:
            ...
            self.assertEqual(category.views, values['views'],
                             f"{FAILURE_HEADER}The number of views for the '{category}' category is incorrect (got {category.views}, "
                             f"expected {values['views']}, generated from populate_rango).{FAILURE_FOOTER}")
            self.assertEqual(category.likes, values['likes'],
                             f"{FAILURE_HEADER}The number of likes for the '{category}' category is incorrect (got {category.likes}, "
                             f"expected {values['likes']}, generated from populate_rango).{FAILURE_FOOTER}")
